...
def register_user(userdata):
    return requests.post(urljoin(SHARED_SERVER_URL, 'users'), json=userdata)
...
def update_user_data(user_id,auth_token,data):
    return requests.put(urljoin(SHARED_SERVER_URL, 'users/{}'.format(user_id)),header={"Authentication":auth_token},json=data)

def validate_user(username, password):
    return requests.post(urljoin(SHARED_SERVER_URL,'users/validate'), json={'username': username, 'password': password})
