...
class RidersAPI(MethodView):
    ...
    def post(username):
        try:
            ...
            data = schema\
                .validate([data])[0]  # IMPORTANTE: el 0 es para que devuelva el diccionario dentro y no una lista
                ...
                if username_user == username:
                    ...
                    if db.requests.count({'username': username, 'pending': True}) == 0:
                        ...
                        result = db.requests.insert_one(
                            {'username': username, 'coordinates': data, 'pending': True})  # TODO: Check this db
                        response = {
                            'status': 'success',
